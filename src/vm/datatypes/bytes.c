...
static Value lenBytes(DictuVM *vm, int argCount, Value *args) {
    if (argCount != 0) {
...
// static Value toNumberBytes(DictuVM *vm, int argCount, Value *args) {
//     if (argCount != 0) {
...
void declareBytesMethods(DictuVM *vm) {
    defineNative(vm, &vm->stringMethods, "len", lenBytes);
